...
contains
  ...
  subroutine basindrain(orderPits, pitID, pyRcv, pIDs, pyDrain, pitNb, pygNodesNb)
        ...
        do n = 1, pitNb
          ...
          donor = nID
          do while(pyDrain(nID)==-1)
            recvr = pyRcv(donor) + 1
            ! If this is an internal basin or an edge node
            if(recvr == donor)then
              pyDrain(nID) = recvr - 1
              count = 1
              chainDrain = -1
  ...
  subroutine qs_max(pyStack, pyRcv, pitID, pitVol, pitDrain, pyXY, pyArea, &
